...
if [[ ! -v ENV_UNIQUE_ID ]]; then
  echo 'the $ENV_UNIQUE_ID variable is not set.'
  echo 'it was canceled.'
  exit 0
fi

readonly RESOURCE_GROUP_FOR_PROVISIONING="rg-${ENV_UNIQUE_ID}-provisioning"
readonly STORAGE_ACCOUNT_FOR_PROVISIONING="$(echo ${ENV_UNIQUE_ID} | tr --complement --delete '0-9a-z' | cut -c-24)"
...
# Registered
# az provider show --namespace Microsoft.Storage | jq -r '.registrationState'
